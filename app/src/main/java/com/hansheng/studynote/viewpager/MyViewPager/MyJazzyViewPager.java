 ...
 * 下面说正确的，其实ViewPager在添加一个View或者销毁一个View时，是我们自己的PageAdapter中控制的，
 * 于是我们可以在ViewPager里面维系一个HashMap<Position，View>，然后滑动的时候，通过get（position）
 * 取出，比如上述效果，始终是右边的View变化，要么从小到大，要么从大到小
 * 那么滑倒下一页：左边的View:map.get(position) ，右边的View : map.get(position+1) .
 * 那么滑倒上一页：左边的View : map.get(position) ， 右边的View : map.get(position+1) ， 一样的，
 * 因为滑到上一页，position为当前页-1
 */
